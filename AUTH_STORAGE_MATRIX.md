...
It separates vendor-documented behavior from places where the storage model is still partly inferred from implementation details.
...
| Claude Code | Linux / WSL | Docs say credentials are "stored securely," but do not explicitly spell out the backend the way they do for macOS | `~/.claude.json` stores OAuth session metadata/caches/per-project state; exact secure credential backend is not explicitly documented on the settings/IAM pages checked | `~/.claude/settings.json`, `.claude/settings.json`, `.claude/settings.local.json`, managed settings under `/etc/claude-code/` | Docs confirm persisted `.jsonl` transcripts via `cleanupPeriodDays`, but not exact directory on the page checked | `~/.claude/agents/`, `~/.claude/CLAUDE.md`, `~/.claude.json`, `.mcp.json` | Medium |
...
| Codex CLI | Linux | Same documented model as macOS: file or OS credential store; `auto` uses OS credential store when available, else `auth.json` | `~/.codex/auth.json` or OS credential store | `~/.codex/config.toml`, `.codex/config.toml`, `/etc/codex/config.toml` | Not explicitly documented in the pages checked | `CODEX_HOME` root override | High |
...
| Gemini CLI | Linux | Same documented model as macOS | Same as macOS | `~/.gemini/settings.json`, `.gemini/settings.json` | `~/.gemini/tmp/<project_hash>/chats/` | `~/.gemini/mcp-oauth-tokens.json`, `~/.gemini/mcp-server-enablement.json` | High |
...
- Linux/Windows: vendor docs do not state the exact backend as clearly; config/state is definitely split across `~/.claude/`, project `.claude/`, and `~/.claude.json`.
...
- Claude: Linux/Windows credential backend is not spelled out as explicitly as macOS Keychain.
