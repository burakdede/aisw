...
#[test]
fn claude_secure_backend_import_use_and_rename_work_end_to_end() {
    let env = TestEnv::new();
    ...
    seed_keychain_item(
        ...
        "tester",
        "{\"token\":\"oauth\",\"email\":\"claude-smoke@example.com\"}",
    );
    ...
    assert!(!env
        ...
        .exists());
    assert!(keychain_secret_path(&env, "aisw", "profile:claude:default").exists());

    secure_cmd_for_tool(&env, "claude")
        .args(["use", "claude", "default"])
        .assert()
        .success()
    ...
    let status_output = secure_cmd_for_tool(&env, "claude")
        .args(["status", "--json"])
        .assert()
        .success()
        .get_output()
        .stdout
        .clone();
    let statuses: serde_json::Value = serde_json::from_slice(&status_output).unwrap();
    let claude = statuses
        .as_array()
        .unwrap()
        .iter()
    ...
    assert!(!keychain_secret_path(&env, "aisw", "profile:claude:default").exists());
    assert!(keychain_secret_path(&env, "aisw", "profile:claude:work").exists());
...
fn codex_secure_backend_lifecycle_supports_backup_restore_end_to_end() {
    ...
    let status_output = secure_cmd_for_tool(&env, "codex")
        .args(["status", "--json"])
        .assert()
        .success()
        .get_output()
    ...
    secure_cmd_for_tool(&env, "codex")
        .args(["remove", "codex", "default", "--yes", "--force"])
        .assert()
        .success()
        .stdout(contains("Removed profile"));
    ...
    cmd_with_secure_env(&env)
        ...
        .stdout(contains("aisw use codex default"));
