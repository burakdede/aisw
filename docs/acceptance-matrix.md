...
| Codex CLI | System keyring with discoverable live account | Supported | Supported | Imports into managed secure storage, keeps managed secret in system keyring, reapplies live keyring secret and keyring config on switch | `tests/secure_backend_cmd.rs::codex_secure_backend_lifecycle_supports_backup_restore_end_to_end`, full `cargo test` |
