...
description: Complete syntax and flag reference for all aisw commands  -  add, context, use, list, status, remove, rename, backup, init, uninstall, shell-hook, and doctor.
---
...
```text
aisw init [--yes]
aisw add <tool> <profile> [--api-key KEY] [--from-env] [--from-live] [--label TEXT] [--credential-backend file|system_keyring] [--set-active] [--yes]
...
- Gemini supports only `file`. Claude and Codex support `file` and `system_keyring`.
