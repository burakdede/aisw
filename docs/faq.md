...
`aisw` manages one active live state per tool. It is designed for deliberate switching and workspace protection; it does not make two already-running Claude processes share or hot-swap credentials. Restart Claude Code after a switch so the new process reads the selected state. If you need independent sessions running concurrently, use Claude Code's own isolated configuration-directory workflow and read [How aisw works](how-it-works.md) to understand the boundary.
...
For API-key profiles, pass the key during `aisw add`. For an account already active in Gemini CLI, use `aisw add gemini <name> --from-live`. Start a new Gemini process after switching; see [Adding profiles](adding-profiles.md) and [Gemini CLI details](supported-tools.md#gemini-cli).
