...
impl BackupManager {
    ...
    /// Snapshot all files in `profile_dir` into a timestamped backup directory.
    ...
    /// Restore files from the backup identified by `timestamp` back into the
    /// corresponding profile directory, enforcing 0600 on all restored files.
    pub fn restore(&self, timestamp: &str, profile_store: &ProfileStore) -> Result<()> {
        let backup_root = self.backups_dir().join(timestamp);
        if !backup_root.is_dir() {
            bail!("no backup found with timestamp '{}'", timestamp);
        }
    ...
    pub fn prune(&self, max_backups: usize) -> Result<()> {
        ...
        for old in timestamps.into_iter().skip(max_backups) {
...
mod tests {
    ...
    fn list_returns_entries_newest_first() {
        ...
        let m = manager(dir.path());
        // Create two snapshots; timestamps are second-granularity so sleep briefly.
    ...
    #[test]
    fn restore_missing_timestamp_errors() {
        let dir = tempdir().unwrap();
        let m = manager(dir.path());
        let ps = profile_store(dir.path());
        let err = m.restore("2099-01-01T00-00-00Z", &ps).unwrap_err();
    ...
    fn restore_writes_files_into_profile_dir() {
        ...
        let entries = m.list().unwrap();
        m.restore(&entries[0].timestamp, &ps).unwrap();
